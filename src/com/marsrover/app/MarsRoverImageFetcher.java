    ...
    public static String formatDate(String date) {
        ...
        try {
            if (inputFormat != null) {
                ...
                // Check if the parsed date is valid for the month/year
                Calendar cal = Calendar.getInstance();
                cal.setTime(parsedDate);

                int day = cal.get(Calendar.DAY_OF_MONTH);
                int month = cal.get(Calendar.MONTH); // Month is 0-based (0=January, 3=April)
                int year = cal.get(Calendar.YEAR);

                // Check if the day is valid for the specific month
                cal.set(year, month, 1); // Set to the 1st day of the month
